...
final class PreviewTestViewController: UIViewController {
    ...
    private var questionDetailView = QuestionDetailView(frame: .zero) // should be modified
    // page inidicaotor
    ...
    private func setOptionActions() {
        option1Label.isUserInteractionEnabled = true
        option2Label.isUserInteractionEnabled = true
        option3Label.isUserInteractionEnabled = true
        option4Label.isUserInteractionEnabled = true
        
        let optionAction1 = UITapGestureRecognizer(target: self, action: #selector(sendOption1TouchEvent))
        let optionAction2 = UITapGestureRecognizer(target: self, action: #selector(sendOption2TouchEvent))
        let optionAction3 = UITapGestureRecognizer(target: self, action: #selector(sendOption3TouchEvent))
        let optionAction4 = UITapGestureRecognizer(target: self, action: #selector(sendOption4TouchEvent))
        
        option1Label.addGestureRecognizer(optionAction1)
        option2Label.addGestureRecognizer(optionAction2)
        option3Label.addGestureRecognizer(optionAction3)
        option4Label.addGestureRecognizer(optionAction4)
    }
    
    @objc private func sendOption1TouchEvent() {
        input.send(.optionSelected(idx: 1))
    }
    @objc private func sendOption2TouchEvent() {
        input.send(.optionSelected(idx: 2))
    }
    @objc private func sendOption3TouchEvent() {
        input.send(.optionSelected(idx: 3))
    }
    @objc private func sendOption4TouchEvent() {
        input.send(.optionSelected(idx: 4))
    }
    ...
    private func setOptionState(idx: Int, isSelected: Bool) {
        ...
        default:
            fatalError("error occured while setting option state in PreviewTestViewController")
        }
    }
    ...
    private func addViews() {
        ...
        questionDetailView.translatesAutoresizingMaskIntoConstraints = false
        ...
        NSLayoutConstraint.activate([
            ...
            //            questionDetailView.leadingAnchor.constraint(equalTo: self.view.leadingAnchor, constant: 18),
            //            questionDetailView.trailingAnchor.constraint(equalTo: self.view.trailingAnchor, constant: -18),
            //            questionDetailView.bottomAnchor.constraint(equalTo: option1Label.topAnchor, constant: -16),
            //            questionDetailView.heightAnchor.constraint(equalToConstant: 300),
